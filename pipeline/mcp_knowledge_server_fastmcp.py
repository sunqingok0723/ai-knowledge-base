#!/usr/bin/env python3
"""MCP Knowledge Server using MCP SDK.

提供本地知识库搜索功能，使用 MCP 官方 SDK。
"""
...
class KnowledgeBase:
    ...
    def _load_articles(self) -> None:
        ...
        if not self.articles_dir.exists():
            print(f"[stderr] Knowledge directory not found: {self.articles_dir}", file=__import__('sys').stderr)
            return
        ...
        for json_file in self.articles_dir.glob("*.json"):
            try:
                ...
                print(f"[stderr] Failed to load {json_file}: {e}", file=__import__('sys').stderr)

        print(f"[stderr] Loaded {len(self.articles)} articles from {self.articles_dir}", file=__import__('sys').stderr)
...
def get_article(article_id: str) -> str:
    ...
    Args:
        article_id: 文章 ID（如：20260514_github_123456）
